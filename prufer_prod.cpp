...
// Быстрое случайное формирование новых пар
List<std::pair<int, int>> generate_new_pairs(int n, const List<std::pair<int, int>>& existing_pairs) {
    ...
    // Создаём список всех свободных индексов
    List<int> available_indices;
    available_indices.reserve(T - used_indices.size());  // Предварительное выделение памяти
    for (int i = 0; i < T; ++i) {
    ...
    if (available_indices.empty()) return {};  // Если новых рёбер нет

    // Определяем случайное количество новых рёбер
    ...
    // Выбираем `l` случайных элементов из `available_indices`
    List<int> chosen_indices(l);
    std::sample(available_indices.begin(), available_indices.end(), chosen_indices.begin(), l, gen);

    // Преобразуем индексы в пары
...
/*************  ✨ Codeium Command ⭐  *************/
/******  c767fea2-5a41-482c-9f5d-6278c2fe0dee  *******/
