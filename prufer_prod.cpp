...
std::vector<SizeType> get_hist(int n, const std::vector<SizeType> &deg, std::vector<SizeType> &hist)
{
    for (auto i : deg)
    {
        hist.at(i)++;
...
void write_vector_to_file(const std::vector<SizeType> &vec, const std::string &filename)
{
    ...
    out.write(reinterpret_cast<const char *>(&size), sizeof(SizeType));             // Записываем размер
    out.write(reinterpret_cast<const char *>(vec.data()), size * sizeof(SizeType)); // Записываем содержимое
    out.close();
}
