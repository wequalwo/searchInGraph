/**
 * Основной файл метода, записывает результат в файл grath.dot
 * v.2.1
 * Метрики (без записи в файл): 
 * 1k вершин: 0.02
 * 10k вершин: 0.7
 * 100k вершин: 25
 */
    ...
    const std::vector<std::pair<uint64_t, uint64_t>>& existing_pairs) 
{
    ...
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<int> dist(0, T - existing_pairs.size() - 1);
    uint64_t l = dist(gen);

    printf("Number of new pairs to generate: l = %llu\n", l);

    std::vector<std::pair<uint64_t, uint64_t>> new_pairs;
    ...
    for (uint64_t i = 1; i <= n; ++i) {
        for (uint64_t j = i + 1; j <= n; ++j) {
            ...
            if (existing_set.count(p)) {
                // Если пара уже есть, просто добавляем её в new_pairs (но без увеличения счётчика)
...
int main(int argc, char* argv[]) {
    ...
    List<std::pair<uint64_t, uint64_t>> all_pairs = edges;  // Все существующие пары
